export default {
  ...
  skills: {
    ...
    descriptions: [
      '熟悉 React 技术栈和相关生态，例如 React Router、Redux 以及其他工具库等；能够使用 Vue 框架及周边全家桶进行开发。',
      '熟悉Javascript、HTML5、CSS3 和 ES6+ 语法，能够使用 TypeScript 开发，提高代码可维护性。',
      '熟悉工程化，掌握 Vite 和 Webpack 使用及配置，具备项目构建与 Git 工作流经验。',
      '熟练使用支付宝小程序 ，灵活掌握支付宝小程序开发框架。', 
      '熟悉 PC、H5、小程序开发，有处理双端兼容性问题的经验，能够准确还原设计稿，处理响应式布局。',
      '熟悉主流组件库和图表库使用和二次封装，例如 Ant Design、Element-UI、ECharts 等。',
      '了解常见网络协议（如HTTP, HTTPS, DNS等）的工作原理及应用场景。',
  ...
  experiences: [
      ...
      type: 'company',
      icon: '/damai.png',
      company: '驻场阿里巴巴-飞猪旅行&&大麦国际',
      position: '前端开发工程师',
      time: '2024 年 05 月 - 至今',
      descriptions: [
        '负责大麦相关h5、小程序、运营平台等多个项目的前端开发',
      ],
    },
    // {
    //   type: 'company',
    ...
    //   position: '前端开发工程师',
    //   time: '2021 年 06 月 - 2022 年 9 月',
    //   descriptions: [
    //     '负责Cleverex教育管理系统的前端开发，维护SSRS报表服务',
    //   ],
    // },
    {
      type: 'company',
    ...
    },
    {
      ...
      descriptions: [
        '中共党员，英语四级证书，普通话二级甲等证书，多门教师资格证书、荣获奖学金'
      ],
    },
  ],
  projects: [
      ...
      type: 'close',
      icon: '/damai.png',
      name: 'maiseat大麦国际h5和小程序的开发',
      description:
      [
        `
        MAISEAT(大麦国际)为跨境观演用户提供票务服务与市场拓展支持,正式上线独立App，iOS版本，并同步推出支付宝小程序，形成覆盖网页、小程序与原生应用的全渠道服务矩阵。平台支持简中、繁中、英、日、韩等多语言界面，集成支付宝全球支付系统实现多币种结算。
        `,
      ],
      stack: ['umi、taro、react、typescript、antd、bui',],
      details: [
        '难点一：开发观演人、地址管理整个功能模块，包括国内以及海外等国家和地区的选择、用户姓名和联系方式，详细地址控件的开发。该功能兼顾了个人中心页和确认订单页的复用性。',
        '难点二：参与开发优惠券相关功能模块，100%还原设计稿样式，兼容pc和h5以及小程序的新人优惠券弹窗中券列表的滚动动效。',
        '亮点一：封装优化SEO的hooks，并且进行td优化，帮助提升了网站的SEO排名和性能。',
        '亮点二：封装公共的可复用的组件，提升代码复用性，严格遵守ts规范进行组件的开发。',
        '亮点三：参与开发小程序嵌套h5，提高小程序的开发效率、提升小程序的用户体验和功能完整性。',
        '亮点四：使用集团内部的埋点组件，对负责功能埋点，有利于分析和统计数据，以此来掌握用户数据和行为，提升产品竞争力。',
      ],
      boldWords:[
        '难点一',
        '亮点一',
    ...
    },
    {
      ...
      description:
      [
        `大麦国际运营平台旨在为大麦国际全平台提供支持，包括项目和场次以及票品的上线和发布，对场馆的管理、票务销售、数据统计等进行全方位管理。`,
      ],
      stack: [
        'react、vue、typescript、antd、微前端'
      ],
      details: [
        '难点一：封装编辑器组件，支持富文本编辑。',
        '难点二：参与场馆管理和场次规划座位和票品的关联。',
        '亮点一：开发门店销售模块的费率计算的功能，前端根据输入的参数动态计算费率，并实时显示结果。',
        '亮点二：项目涉及微前端框架，子应用使用的技术分别为react和vue，提升项目模块化和可维护性。',
      ],
      boldWords:[
        '亮点一',
        '亮点二',
        '难点一',
        '难点二',
        '前端根据输入的参数动态计算费率，并实时显示结果。',
        '项目涉及微前端框架',
        '子应用使用的技术分别为react和vue',
      ]
    },
    {
      type: 'close',
      ...
      details: [
        '难点一：引入智能决策引擎和异常处理机制，以快速响应并处理各种特殊情况，提升整体交易处理效率与准确性需要处理复杂的正向和逆向的交易链路。',
        '难点二：基于map-guide插件的对现有地图功能进行优化，通过客户端缓存，数据预处理等方式解决路线请求加载过慢等问题。',
        '亮点一：参与了公司/项目通用公共组件库的封装工作，负责设计并实现了一系列可复用、高性能的UI组件。在封装过程中，深入研究了组件的架构设计，确保组件的易用性、可扩展性和维护性。',
        '亮点二：双端兼容性：处理不同机型的兼容性问题，尤其是在iOS和Android之间，涉及字体、布局等渲染问题。',
        '成果：优化产品功能和用户体验，成功实现了月活跃用户提升35%的显著增长。同时，用户满意度也上升了20%，在提升用户粘性和满意度方面取得重大突破。',
      ],
      boldWords:[
        '核心项目',
        '智能决策引擎和异常处理机制',
    ...
    },
    {
      ...
      details: [
        '难点一：基于iframe以及arale-qrcode（用来生成二维码）的预览组件，在B端可以直接预览在C端的实现效果。',
        '难点二：通过重构模块与使用 Form Render 优化表单，减少冗余代码，提高开发人员的工作效率，节省5人日人工成本。',
        '亮点一：通过模块化设计、配置化驱动，设计灵活的数据模型，实现票种、票型、点位配置等功能模块及复杂的数据关系和交互。',
        '亮点二：通过参数化设计、状态同步、错误处理等方式抽取相似逻辑为公共 hooks，例如UseLocalStorage，存储和获取本地存储数据。',
    ...
    },
    {
      ...
      details: [
        '难点一：基于Webpack构建工程，通过其强大的插件系统和loader机制，在构建过程中实现文件级别的条件编译，提高构建速度、减小输出文件大小、提升应用性能和可靠性，同时支持按需加载。',
        '难点二：重构部分模块，使用React Hooks（如useMemo、useCallback等）进行优化，考虑性能的高效性和可靠性，避免潜在的内存泄露和性能下降。',
        '难点三：封装各模块复杂数据清洗函数，定义清洗规则，实现代码的高内聚和低耦合，以确保可复用性和可读性。',
        '亮点一：确保浏览器兼容性，特别是针对老旧版本浏览器的细致适配，显著增强了用户体验的广泛性和用户认可度。',
        '亮点二：主导设计BI看板模块，将复杂数据以图形和图表的形式呈现，帮助员工更快地理解和分析信息，从而快速做出决策，各项业务流程的办理时间缩短，显著提升员工工作效率。'
      ],
      boldWords:[
        '各项业务流程的办理时间缩短30%',
        '难点一',
    ...
    //     '难点四：使用 sql sever 编写自动化脚本向数据库中录入信息，确保数据库操作没有遗漏和错误。',
